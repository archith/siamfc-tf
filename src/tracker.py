...
#def tracker(hp, run, design, frame_name_list, pos_x, pos_y, target_w, target_h, final_score_sz, filename, image, templates_z, scores, start_frame, gpu_id):
def tracker(hp, run, design, frame_name_list_list, pos_x_list, pos_y_list, target_w_list, target_h_list, final_score_sz, filename, image, templates_z, scores, start_frame, gpu_id):    
    ...
    with tf.Session(config=config) as sess:
        ...
        for idx in range(0, len(frame_name_list_list)):
            if idx%10 == 0:
                print '\r %04d/%04d'%(idx, len(frame_name_list_list)),
                sys.stdout.flush()

            frame_name_list = frame_name_list_list[idx]
            pos_x = pos_x_list[idx]
            pos_y = pos_y_list[idx]
            target_w = target_w_list[idx]
            target_h = target_h_list[idx]
